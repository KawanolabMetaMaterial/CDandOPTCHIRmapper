...
import matplotlib.patches as patches # 円描画用に追記
import sys
...
# 基準Optical Chirality計算用 (追記部分)
def OptChir_ref(omega, E0=1.0):
    """
    基準となるOptical Chirality Cを計算する。
    定義: C = (epsilon_0 * omega / c) * E0^2
    """
    # omegaは配列で来る可能性があるため、定数計算のみ行う
    print((epsilon_0 * omega / c) * (E0 ** 2))
    return (epsilon_0 * omega / c) * (E0 ** 2)
...
def Licence():
    ...
    CopyLightLabel = Label(Lframe, justify='center', text='バージョン 1.10 \n(C) 2022 小西優一 \nスペシャルサンクス: 大川拓樹 橋谷田俊 西山黎 \n 中央大学 河野研究室', font=('Source Han Serif', '10'))
...
class CSVmanipulator:
    ...
    def E_ReferenceButton(self):
        ...
        if Efile is not None: # ファイルが選択された場合のみ処理
            if '.csv' not in Efile.name: # 拡張子に.csvが入っていないとき
                ...
                if error == False: # いいえを選択したとき
                    self.E_ReferenceButton() # 再帰呼び出し（selfが必要）
                elif error is None: # キャンセルしたとき
                    pass
                else:
                    E_Inptfile.set(Efile.name)
                    Efile.close() # closeはメソッドなので()が必要
                    self.E_READBUTTON = 1
    ...
    def ReadCSV(self, E_CSVstr, M_CSVstr):
        ...
        if E_CSVstr != '' and M_CSVstr != '': #内容のある文字列が渡されたとき
            ...
            OptChrres = OptChr(FreqHz, OCxres, OCyres, OCzres)

            # --- 増強度 (Enhancement) の計算を追加 ---
            omega = 2 * np.pi * FreqHz
            C_ref = OptChir_ref(omega, E0=1.0)
            EnhancementRes = OptChrres / C_ref
            # ----------------------------------------
            ...
            # 増強度カラム追加
            EnhancementDF = pd.DataFrame(EnhancementRes, columns=['Optical Chirality Enhancement'])
            ...
            CDdata = pd.concat([CDdata, EnhancementDF], axis=1) # 増強度を結合

            print(CDdata) # 確認用

            self.GCDdata = CDdata # class内変数にCDdataを渡す
            gb['state'] = 'normal' # グラフ描画ボタンを有効化する
            ref_gb['state'] = 'normal' # リファレンスグラフボタンを有効化する
    ...
    # --- 新しく追加したリファレンス用プロット関数 ---
    def GenerateRefGraph(self):
        # 入力された半径を取得（未入力の場合は0とするかエラー処理）
        try:
            radius_mm = float(radius_entry.get())
        except ValueError:
            messagebox.showerror("エラー", "半径には数値を入力してください")
            return

        # CSVの対象列を格納
        X = self.GCDdata[' X [mm]'].values
        Y = self.GCDdata[' Y [mm]'].values
        C_enh = self.GCDdata['Optical Chirality Enhancement'].values # 増強度を使用

        # カラーバーのレンジ調整（増強度なので絶対値やスケール感は異なるが、同様に処理）
        Cmax = max(C_enh)
        Cmin = min(C_enh)
        # 0中心にするか、正の値のみかによりますが、一旦絶対値最大で正規化
        limit = max(abs(Cmin), abs(Cmax))

        plt.figure(f"Enhancement (r={radius_mm}mm)")
        ax = plt.gca()

        # ヒートマップ
        im = plt.imshow(TwoColumn2Array(X, Y, C_enh), cmap='jet', extent=(min(Y), max(Y), max(X), min(X)))
        plt.colorbar(im, label='Enhancement ($C/C_{ref}$)')
        
        # 円形開口の描画 (点線)
        # グラフの軸はmm単位なので、半径もmmのままでOK
        aperture = patches.Circle((0, 0), radius_mm, linewidth=1.5, edgecolor='white', facecolor='none', linestyle='--', label=f'Aperture (r={radius_mm}mm)')
    ...
    def CSVFileViewer(self):
        ...
        # レコード追加（重くなるので先頭100行程度にするか、そのままにするか。現状は全件）
        # データが多いと起動が遅くなる可能性があります
        ...
        self.tree.grid(row=4, column=0, columnspan=4, sticky= [N, E, W, S]) # Rowをずらしました
...
root.minsize(1440, 450) # 高さを少し広げました

E_Inptfile = StringVar()
M_Inptfile = StringVar()
radius_input = StringVar() # 半径入力用変数
...
Inptfile_entry = ttk.Entry(
    frame1, textvariable=E_Inptfile, width=150
)
...
# Path Entry Fourm*****************************
Inptfile_entry = ttk.Entry(
    frame1, textvariable=M_Inptfile, width=150
)
...
CommandButtonFrame.grid(row=2, column=0, columnspan=4, pady=10)
...
readb.grid(row=0, column=0, padx=5)

# Generate Graph Button************************
gb = ttk.Button(
    CommandButtonFrame, text='グラフ生成(生データ)', command=lambda: CM.GenerateGraph()
)
gb.state(['disabled']) # 無効にする
gb.grid(row=0, column=1, padx=5)

# --- 新しいUI部分: 半径入力とRefプロットボタン ---
radius_label = ttk.Label(CommandButtonFrame, text="開口半径[mm]:")
radius_label.grid(row=0, column=2, padx=(20, 2))
radius_entry = ttk.Entry(CommandButtonFrame, textvariable=radius_input, width=5)
radius_entry.grid(row=0, column=3, padx=2)
radius_entry.insert(0, "3.25") # デフォルト値

ref_gb = ttk.Button(
    CommandButtonFrame, text='Reference付きプロット', command=lambda: CM.GenerateRefGraph()
)
ref_gb.state(['disabled']) # 初期状態は無効
ref_gb.grid(row=0, column=4, padx=5)
# -----------------------------------------------

# Save CSV File Button*************************
Scsvb = ttk.Button(
    CommandButtonFrame, text='CSVファイルを保存', command=lambda: CM.SaveCSVFile()
)
Scsvb.state(['disabled'])
Scsvb.grid(row=0, column=5, padx=5)
# *********************************************
